...
const SideBar = () => {
    ...
    return (
        <>
            ...
            <div className="md:hidden fixed top-0 left-0 right-0 z-40 bg-[var(--color-background-white)] border-b border-[var(--color-border-gray)] px-4 h-16 flex items-center justify-between shadow-sm">
            ...
            {/* Sidebar */}
            <aside className={`
                w-64 bg-[var(--color-background-white)] border-r border-[var(--color-border-gray)] 
                fixed left-0 top-0 h-full overflow-y-auto z-50
                transition-transform duration-300 ease-in-out
                ${isMobileOpen ? 'translate-x-0' : '-translate-x-full'}
                md:translate-x-0
            `}>
                <div className="p-6 border-b border-[var(--color-border-gray)] flex justify-between items-start">
